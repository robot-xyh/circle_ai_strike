...
namespace {
constexpr char kMagic[8] = {'C', 'P', 'S', 'T', 'R', 'P', 'A', '1'};
constexpr uint32_t kVersion = 1;
}  // namespace

struct StrikeParamsSnapshotWriter::Header {
  char magic[8];
  uint32_t version;
  uint32_t capacity;
...
struct StrikeParamsSnapshotReader::Header {
  char magic[8];
  uint32_t version;
  uint32_t capacity;
  uint32_t payload_bytes;
...
StrikeParamsSnapshotWriter::~StrikeParamsSnapshotWriter() { close(); }

bool StrikeParamsSnapshotWriter::open(const std::string& name,
...
void StrikeParamsSnapshotWriter::close() {
  if (mapping_) {
...
bool StrikeParamsSnapshotWriter::writeJson(const std::string& json) {
...
StrikeParamsSnapshotReader::~StrikeParamsSnapshotReader() { close(); }

bool StrikeParamsSnapshotReader::open(const std::string& name) {
  close();
...
void StrikeParamsSnapshotReader::close() {
  if (mapping_) {
    ::munmap(mapping_, mapping_size_);
    mapping_ = nullptr;
  }
...
uint64_t StrikeParamsSnapshotReader::currentSeq() const {
  if (!header_) {
    return 0;
  }
  return header_->seq;
...
bool StrikeParamsSnapshotReader::readCurrentJson(std::string& json_out) const {
  if (!header_ || !payload_ || header_->seq == 0 ||
      header_->payload_bytes == 0) {
    return false;
  }
  const uint32_t n =
      std::min(header_->payload_bytes, header_->capacity);
